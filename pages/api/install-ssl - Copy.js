  ...
  if (req.method === 'POST') {
    ...
    try {
      for (const { alias, path: certPath, enabled } of certPaths) {
        ...
        const fullCertPath = path.join(certsDir, certPath);
        const commandOutput = await new Promise((resolve, reject) => {
          ...
          process.on('close', (code) => {
            if (code !== 0) reject(`Failed to install ${alias}\nOutput: ${output}`);
      ...
      return res.status(200).json({ results, file: keystoreName });
    } catch (error) {
      return res.status(500).json({ message: `Error: ${error}` });
    }
  } else if (req.method === 'GET') {
    try {
      const keystoreFiles = fs.readdirSync(filesDir)
        .filter(file => file.endsWith('.keystore'));
